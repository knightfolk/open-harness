...
import { createAutoRouterCandidateEvidenceBuilder, routerModelKeysMatch } from '../utils/autoRouterCandidateEvidence';
import { promptStrategyIdForModel } from '../utils/promptStrategyResolver';
import { activeOlderSameProviderMiniMaxCandidateIds as activeOlderSameProviderMiniMaxCandidateIdsForRouter, filterAutoGeneratedMiniMaxRouterCandidates, formatMiniMaxSuppressedCandidateList, pickPreferredMiniMaxM3RouterCandidate, summarizeAutoGeneratedMiniMaxRouterCandidatePreference, type MiniMaxRouterCandidatePreferenceSummary } from '../utils/minimaxRouterCandidatePreference';
import { activeOlderSameProviderGlmCandidateIds as activeOlderSameProviderGlmCandidateIdsForRouter, filterAutoGeneratedGlmRouterCandidates, formatGlmSuppressedCandidateList, summarizeAutoGeneratedGlmRouterCandidatePreference, type GlmRouterCandidatePreferenceSummary } from '../utils/glmRouterCandidatePreference';
import { chooseAutoRouterRecommendedSelection } from '../utils/onboardingModelPreference';
import { glm52PreferenceNoticeLabel, glm52SuppressionReason, glmActiveOlderRoutingWarning, glmPatientPartnerLabel, glmPatienceCandidateStatusLabel, glmPatienceLaneLabel, glmPatienceSettingsIntro, glmPatienceSettingsTitle, isGlm5ModelId, isGlm52ModelId } from '../../shared/glmModelPreference';
import { miniMaxM3ActiveOlderRoutingWarning, miniMaxM3PreferenceNoticeLabel, miniMaxM3PreferencePolicyLabel, miniMaxM3SuppressionReason } from '../../shared/minimaxModelPreference';
...
const PROVIDER_PRESETS: ProviderPreset[] = [
  ...
  { id: 'minimax', name: 'MiniMax', type: 'openai-compatible', baseURL: 'https://api.minimax.io/v1', description: miniMaxM3PreferencePolicyLabel(), color: '#6366f1', featured: true },
  ...
  { id: 'zhipu', name: 'Z.AI / Zhipu', type: 'openai-compatible', baseURL: 'https://api.z.ai/api/coding/paas/v4', description: glmPatientPartnerLabel('glm-5.2'), color: '#3b5998', featured: true },
...
const MODEL_RECOMMENDATIONS: Record<string, string[]> = {
  planner: ['o3', 'glm-5.2', 'deepseek-r2', 'deepseek-v4', 'llama-4-scout', 'kimi-k2.5', 'gpt-5.4', 'MiniMax-M3'],
  coder: ['deepseek-v4', 'gpt-4.1', 'llama-4-maverick', 'MiniMax-M3', 'qwen-3-235b', 'glm-5.2', 'kimi-k2.6', 'grok-3', 'codestral', 'gpt-5.3-codex'],
  reviewer: ['o3', 'glm-5.2', 'mistral-large', 'o4-mini', 'deepseek-r2', 'qwen-3-235b', 'mimo-v2.5-pro'],
  reasoner: ['o3', 'glm-5.2', 'deepseek-r2', 'qwen-3-235b', 'grok-3', 'MiniMax-M3'],
  summarizer: ['gpt-4.1-mini', 'deepseek-v4-flash', 'qwen-3-32b', 'mistral-small', 'MiniMax-M3'],
  worker: ['gpt-4.1-nano', 'deepseek-v4-flash', 'qwen-3-32b', 'MiniMax-M3'],
};
...
function isModelRecommended(roleId: string, modelId: string): boolean {
  ...
  const normalizedModel = lower.replace(/[-\s]/g, '');
  return recs.some((rec) => normalizedModel.includes(rec.toLowerCase().replace(/[-\s]/g, '')));
}
        ...
        <div className="settings-modal-body">
          ...
          <div className="settings-content">
            <SettingsPaneErrorBoundary key={contentKey}>
              ...
              {contentKey === 'assistant/memory' && <AssistantMemoryPane workingDir={workingDir} />}
              ...
              {contentKey === 'auto-router' && <AutoRouterPane onSelectModel={onSelectModel} />}
...
function ModelLibraryPane({ providers }: { providers: ProviderConfig[] }) {
  ...
  const enabledProviderModels = useMemo(() => (
    providers.flatMap((provider) => (
      provider.configured ? provider.models.filter((model) => model.enabled) : []
    ))
  ), [providers]);
  const categoryCounts = useMemo(() => TOP_MODEL_CATALOG.reduce<Record<string, number>>((acc, card) => {
    const category = modelBestCategory(card);
    acc[category] = (acc[category] || 0) + 1;
    return acc;
  }, {}), []);
  const freshnessCounts = useMemo(() => TOP_MODEL_CATALOG.reduce<Record<string, number>>((acc, card) => {
    const status = modelCatalogFreshness(card).status;
    acc[status] = (acc[status] || 0) + 1;
    return acc;
  }, {}), []);

  const filtered = useMemo(() => TOP_MODEL_CATALOG.filter((card) => {
  ...
  }), [accessOnly, categoryFilter, providers, query]);
...
function routerToolReliabilityForModel(
  ...
  for (const [model, bucket] of Object.entries(byModel)) {
    if (routerModelKeysMatch(modelId, model)) {
      return bucket;
    }
  }
  return null;
...
function routerToolRecoveryForModel(
  ...
  return examples.find((example) => {
    return routerModelKeysMatch(modelId, example.firstError.model);
  }) || null;
}
...
function routerRetryReductionForModel(
  ...
  return recommendations.find((recommendation) => {
    return routerModelKeysMatch(modelId, recommendation.failedModel);
  }) || null;
}

function modelRecommendationMatches(candidateModelId: string, recommendationModelId: string): boolean {
  return routerModelKeysMatch(candidateModelId, recommendationModelId);
}
...
function routerToolPairRisksForModel(
  ...
  return Object.entries(pairs)
    .filter(([pair, stats]) => {
      ...
      return stats.error > 0 && routerModelKeysMatch(modelId, modelPart);
    })
...
function getEffectiveRouterCost(modelId: string, providerId: string, baseCost: number, accessMode?: string) {
  ...
  if (providerId === 'z-ai-zhipu' || providerId === 'zhipu') {
    if (lower.includes('glm-5.2')) return 0.05;
    if (lower.includes('glm-5')) return 0.08;
    if (lower.includes('glm-4.7')) return 0.12;
    return 0.1;
  }
...
interface ConfiguredRouterCandidateScan {
  candidates: api.AutoRouterCandidateConfig[];
  miniMaxPreference: MiniMaxRouterCandidatePreferenceSummary;
  glmPreference: GlmRouterCandidatePreferenceSummary;
}

const EMPTY_MINIMAX_ROUTER_CANDIDATE_PREFERENCE: MiniMaxRouterCandidatePreferenceSummary = {
  keptCount: 0,
  suppressedCount: 0,
  suppressedModelIds: [],
...
const EMPTY_GLM_ROUTER_CANDIDATE_PREFERENCE: GlmRouterCandidatePreferenceSummary = {
  keptCount: 0,
  suppressedCount: 0,
  suppressedModelIds: [],
  preferredProviderIds: [],
...
function scanConfiguredRouterCandidates(cfg: api.AppConfig | null): ConfiguredRouterCandidateScan {
  if (!cfg) return {
    candidates: [],
    miniMaxPreference: EMPTY_MINIMAX_ROUTER_CANDIDATE_PREFERENCE,
    glmPreference: EMPTY_GLM_ROUTER_CANDIDATE_PREFERENCE,
  };
  ...
  const miniMaxPreference = summarizeAutoGeneratedMiniMaxRouterCandidatePreference(candidates);
  const glmPreference = summarizeAutoGeneratedGlmRouterCandidatePreference(candidates);
  const seen = new Set<string>();
  const miniMaxFilteredCandidates = filterAutoGeneratedMiniMaxRouterCandidates(candidates, miniMaxPreference);
  const glmFilteredCandidates = filterAutoGeneratedGlmRouterCandidates(candidates, glmPreference);
  const filteredCandidates = glmFilteredCandidates
    .filter((candidate) => miniMaxFilteredCandidates.some((item) => normalizeModelKey(item.modelId) === normalizeModelKey(candidate.modelId)))
    .sort((a, b) => {
...
function uniqueGlmCandidateIds(candidates: api.AutoRouterCandidateConfig[]): string[] {
  const seen = new Set<string>();
  const ids: string[] = [];
  for (const candidate of candidates) {
    if (!isGlm5ModelId(candidate.modelId)) continue;
    const key = normalizeModelKey(candidate.modelId);
    if (seen.has(key)) continue;
    seen.add(key);
    ids.push(candidate.modelId);
  }
  return ids;
}

function AutoRouterPane({ onSelectModel }: { onSelectModel: (modelId: string) => void }) {
  ...
  const [configuredMiniMaxPreference, setConfiguredMiniMaxPreference] = useState<MiniMaxRouterCandidatePreferenceSummary>(EMPTY_MINIMAX_ROUTER_CANDIDATE_PREFERENCE);
  const [configuredGlmPreference, setConfiguredGlmPreference] = useState<GlmRouterCandidatePreferenceSummary>(EMPTY_GLM_ROUTER_CANDIDATE_PREFERENCE);
  const [evalRecommendations, setEvalRecommendations] = useState<api.EvalRecommendation[]>([]);
  ...
  const [routerDefaultsStatus, setRouterDefaultsStatus] = useState<{ tone: 'success' | 'warning' | 'error'; message: string } | null>(null);
  const configuredMiniMaxSuppressedCandidates = useMemo(
    () => formatMiniMaxSuppressedCandidateList(configuredMiniMaxPreference.suppressedModelIds),
    [configuredMiniMaxPreference.suppressedModelIds],
  );
  const configuredGlmSuppressedCandidates = useMemo(
    () => formatGlmSuppressedCandidateList(configuredGlmPreference.suppressedModelIds),
    [configuredGlmPreference.suppressedModelIds],
  );
  ...
  useEffect(() => {
    ...
    api.getConfig().then(async (cfg) => {
      const scan = scanConfiguredRouterCandidates(cfg);
      const scannedCandidates = scan.candidates;
      setConfiguredMiniMaxPreference(scan.miniMaxPreference);
      setConfiguredGlmPreference(scan.glmPreference);
      ...
      if (cfg?.autoRouter) {
        ...
        const recommended = chooseAutoRouterRecommendedSelection(scannedCandidates);
        setArClassifier(recommended.classifierModel);
        setArDefaultModel(recommended.defaultModel);
  ...
  const clearRouterDefaultsStatus = () => setRouterDefaultsStatus(null);

  const toggleAutoRouter = async () => {
    setArSaving(true);
    clearRouterDefaultsStatus();
    try {
      if (arEnabled) {
        ...
        const recommended = chooseAutoRouterRecommendedSelection(candidates);
        const merged = await api.configureRouter({
          enabled: true,
          classifierModel: currentAr?.classifierModel || arClassifier || recommended.classifierModel,
          threshold: currentAr?.threshold ?? arThreshold,
          defaultModel: currentAr?.defaultModel || arDefaultModel || recommended.defaultModel,
          cacheTTLMs: currentAr?.cacheTTLMs ?? 300000,
          candidates,
        });
        setConfiguredMiniMaxPreference(scan.miniMaxPreference);
  ...
  const applyRecommendedRouterDefaults = async () => {
    setArSaving(true);
    setRouterDefaultsStatus(null);
    try {
      const cfg = await api.getConfig();
      const scan = scanConfiguredRouterCandidates(cfg);
      const candidates = scan.candidates;
      const recommended = chooseAutoRouterRecommendedSelection(candidates, cfg?.activeModel || '');
      if (!candidates.length || !recommended.classifierModel || !recommended.defaultModel) {
        setRouterDefaultsStatus({ tone: 'warning', message: 'No configured models available for recommended Auto-Router defaults.' });
        return;
      }

      const result = await api.configureRouter({
        enabled: true,
        classifierModel: recommended.classifierModel,
        threshold: cfg?.autoRouter?.threshold ?? arThreshold,
        defaultModel: recommended.defaultModel,
        cacheTTLMs: cfg?.autoRouter?.cacheTTLMs ?? 300000,
        candidates,
      });
      setConfiguredMiniMaxPreference(scan.miniMaxPreference);
      setConfiguredGlmPreference(scan.glmPreference);
      setConfiguredCandidates(candidates);
      setArCandidates(candidates);
      setArClassifier(recommended.classifierModel);
      setArDefaultModel(recommended.defaultModel);
      setArThreshold(result.state.threshold);
      setArEnabled(true);
      setRouterState(result.state);
      onSelectModel('Auto');
      setRouterDefaultsStatus({
        tone: 'success',
        message: `Recommended defaults applied. Classifier ${recommended.classifierModel}; default ${recommended.defaultModel}.`,
      });
    } catch (err) {
      console.error('Failed to apply recommended auto-router defaults:', err);
      setRouterDefaultsStatus({ tone: 'error', message: 'Recommended defaults failed. Check configured providers and try again.' });
    } finally {
      setArSaving(false);
    }
  };
  ...
  const syncConfiguredCandidates = async () => {
    ...
    try {
      ...
      const syncPreferredCandidateModelId = pickPreferredMiniMaxM3RouterCandidate(updated);
      const syncPreferredGlmCandidateModelId = updated.find((candidate) => isGlm52ModelId(candidate.modelId))?.modelId || '';
      const nextClassifier = resolveRouterSelection(arClassifier, updated, syncPreferredCandidateModelId);
      const nextDefaultModel = resolveRouterSelection(arDefaultModel, updated, syncPreferredGlmCandidateModelId || nextClassifier);
      setConfiguredCandidates(scannedCandidates);
      setConfiguredMiniMaxPreference(scan.miniMaxPreference);
      setConfiguredGlmPreference(scan.glmPreference);
      setArCandidates(updated);
  ...
  const routerCandidateEvidenceBuilder = useMemo(() => createAutoRouterCandidateEvidenceBuilder(routerLearningSummary), [routerLearningSummary]);
  const activeGlmCandidateIds = useMemo(() => uniqueGlmCandidateIds(arCandidates), [arCandidates]);
  const configuredGlmCandidateIds = useMemo(() => uniqueGlmCandidateIds(configuredCandidates), [configuredCandidates]);
  const activeOlderGlmCandidateIds = useMemo(
    () => activeOlderSameProviderGlmCandidateIdsForRouter(arCandidates, configuredCandidates),
    [arCandidates, configuredCandidates],
  );
  const activeOlderMiniMaxCandidateIds = useMemo(
    () => activeOlderSameProviderMiniMaxCandidateIdsForRouter(arCandidates, configuredCandidates),
    [arCandidates, configuredCandidates],
  );
  const miniMaxPreferenceStatusParts = [
    configuredMiniMaxPreference.suppressedCount > 0
      ? `Suppressed ${configuredMiniMaxPreference.suppressedCount} auto-generated MiniMax M2.x candidate${configuredMiniMaxPreference.suppressedCount === 1 ? '' : 's'}${configuredMiniMaxSuppressedCandidates.accessibilityText ? `: ${configuredMiniMaxSuppressedCandidates.accessibilityText}` : ''}`
      : null,
    activeOlderMiniMaxCandidateIds.length > 0
      ? `Active older MiniMax still routed: ${activeOlderMiniMaxCandidateIds.join(', ')}`
      : null,
  ].filter((part): part is string => Boolean(part));
  const glmPatienceCandidateIds = useMemo(() => uniqueGlmCandidateIds([...arCandidates, ...configuredCandidates]), [arCandidates, configuredCandidates]);
  const glmPatienceReferenceModel = glmPatienceCandidateIds.find(isGlm52ModelId) || glmPatienceCandidateIds[0] || '';
  const glmPatienceStatusParts = [
    glmPatienceCandidateStatusLabel(glmPatienceReferenceModel, glmPatienceCandidateIds.length, activeGlmCandidateIds.length, configuredGlmCandidateIds.length),
    activeOlderGlmCandidateIds.length > 0
      ? `Active older GLM still routed: ${activeOlderGlmCandidateIds.join(', ')}`
      : null,
  ].filter((part): part is string => Boolean(part));
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 2 }}>
          <div className="role-recommendation-bar">
            <div>
              <div className="role-recommendation-title">M3 default + cheap classifier</div>
              <div className="role-recommendation-copy">Rebuilds candidates from configured models, keeps MiniMax M3 as the fallback default, uses the lowest-cost classifier, switches active model to Auto, and replaces manual candidate edits.</div>
              {routerDefaultsStatus && (
                <div
                  className={`eval-proof-status ${routerDefaultsStatus.tone === 'success' ? 'approved' : routerDefaultsStatus.tone === 'error' ? 'needs-attention' : 'unreviewed'}`}
                  role="status"
                  aria-live="polite"
                  aria-label={`Auto-Router recommended defaults status: ${routerDefaultsStatus.message}`}
                  style={{ marginTop: 6, fontSize: 11 }}
                >
                  {routerDefaultsStatus.message}
                </div>
              )}
            </div>
            <button
              type="button"
              className="settings-mini-button"
              onClick={applyRecommendedRouterDefaults}
              disabled={arSaving}
              aria-label="Restore recommended Auto-Router defaults"
            >
              <Sparkles size={12} aria-hidden="true" /> Restore recommended defaults
            </button>
          </div>
          ...
          {glmPatienceCandidateIds.length > 0 && (
            <div
              className="router-eval-recommendation"
              role="status"
              aria-live="polite"
              aria-atomic="true"
              aria-label={glmPatienceStatusParts.join('. ')}
              title={glmPatienceSettingsTitle(glmPatienceReferenceModel)}
              style={{ marginTop: 8, marginBottom: 8 }}
            >
              <span>{glmPatienceLaneLabel(glmPatienceReferenceModel)}</span>
              <p>{glmPatienceSettingsIntro()}</p>
              <p className="eval-proof-status unreviewed">
                Active GLM: {activeGlmCandidateIds.length ? activeGlmCandidateIds.join(', ') : 'none yet'}.
                {' '}Configured GLM: {configuredGlmCandidateIds.length ? configuredGlmCandidateIds.join(', ') : 'none yet'}.
              </p>
              {activeOlderGlmCandidateIds.length > 0 && (
                <p className="eval-proof-status unreviewed">
                  Active older GLM still routed: {activeOlderGlmCandidateIds.join(', ')}. {glmActiveOlderRoutingWarning()}
                </p>
              )}
            </div>
          )}
          {(configuredMiniMaxPreference.suppressedCount > 0 || activeOlderMiniMaxCandidateIds.length > 0) && (
            <div
              className="router-eval-recommendation"
              role="status"
              aria-label={[miniMaxM3PreferenceNoticeLabel(), ...miniMaxPreferenceStatusParts].join('. ')}
              title={configuredMiniMaxSuppressedCandidates.fullText}
              style={{ marginTop: 8, marginBottom: 8 }}
            >
              <span>{miniMaxM3PreferenceNoticeLabel()}</span>
              {configuredMiniMaxPreference.suppressedCount > 0 && (
                <>
                  <p>
                    Suppressed {configuredMiniMaxPreference.suppressedCount} auto-generated MiniMax M2.x candidate{configuredMiniMaxPreference.suppressedCount === 1 ? '' : 's'} {miniMaxM3SuppressionReason()}.
                  </p>
                  <p className="eval-proof-status unreviewed">
                    Preferred M3 provider{configuredMiniMaxPreference.preferredProviderIds.length === 1 ? '' : 's'}: {configuredMiniMaxPreference.preferredProviderIds.join(', ')}.
                  </p>
                  <p className="eval-proof-status unreviewed">
                    Hidden from auto-generated sync: {configuredMiniMaxSuppressedCandidates.visibleText}. Manual candidates and different-provider MiniMax fallbacks are preserved.
                  </p>
                </>
              )}
              {activeOlderMiniMaxCandidateIds.length > 0 && (
                <p className="eval-proof-status unreviewed">
                  Active older MiniMax still routed: {activeOlderMiniMaxCandidateIds.join(', ')}. {miniMaxM3ActiveOlderRoutingWarning()}
                </p>
              )}
            </div>
          )}
          {configuredGlmPreference.suppressedCount > 0 && (
            <div
              className="router-eval-recommendation"
              role="status"
              aria-label={`${glm52PreferenceNoticeLabel()} suppressed ${configuredGlmPreference.suppressedCount} auto-generated older GLM candidate${configuredGlmPreference.suppressedCount === 1 ? '' : 's'}: ${configuredGlmSuppressedCandidates.accessibilityText}`}
              title={configuredGlmSuppressedCandidates.fullText}
              style={{ marginTop: 8, marginBottom: arEnabled ? 0 : 8 }}
            >
              <span>{glm52PreferenceNoticeLabel()}</span>
              <p>
                Suppressed {configuredGlmPreference.suppressedCount} auto-generated older GLM candidate{configuredGlmPreference.suppressedCount === 1 ? '' : 's'} {glm52SuppressionReason()}.
              </p>
              <p className="eval-proof-status unreviewed">
                Preferred GLM-5.2 provider{configuredGlmPreference.preferredProviderIds.length === 1 ? '' : 's'}: {configuredGlmPreference.preferredProviderIds.join(', ')}.
              </p>
              <p className="eval-proof-status unreviewed">
                Hidden from auto-generated sync: {configuredGlmSuppressedCandidates.visibleText}. Manual candidates and different-provider GLM fallbacks are preserved.
              </p>
            </div>
          )}
          {arEnabled && (
            <div className="settings-card" style={{ marginTop: 8, padding: 12 }}>
              ...
              {thresholdAdvice && (
                <div
                  className="router-eval-recommendation"
                  role="status"
                  aria-label={`Auto-Router threshold advice: configured ${formatScoreDisplay(thresholdAdvice.configuredThreshold)}, active ${formatScoreDisplay(thresholdAdvice.activeThreshold)}, suggested ${formatScoreDisplay(thresholdAdvice.suggestedThreshold)}, ${thresholdAdvice.dataPoints} rated outcomes, ${thresholdAdvice.applied ? 'applied' : 'not applied'}`}
                  style={{ marginBottom: 10 }}
                >
                  <span>Learned threshold advice</span>
                  <p>
                    Configured {formatScoreDisplay(thresholdAdvice.configuredThreshold)}, active {formatScoreDisplay(thresholdAdvice.activeThreshold)}, suggested {formatScoreDisplay(thresholdAdvice.suggestedThreshold)}
                    {' '}from {thresholdAdvice.dataPoints} rated outcome{thresholdAdvice.dataPoints === 1 ? '' : 's'}.
                  </p>
                  <p className={`eval-proof-status ${thresholdAdvice.applied ? 'approved' : 'unreviewed'}`}>
                    {thresholdAdvice.applied ? 'Applied to this runtime threshold.' : 'Not applied to this runtime threshold.'} {thresholdAdvice.reason}
                  </p>
                  {thresholdAdvice.slowTimingContext && (
                    <p className="eval-proof-status unreviewed">
              ...
              >
                ...
                {routerLearningEvidenceStatusLabel && (
                  <p
                    className={`settings-router-learning-status settings-router-learning-status-${routerLearningSummaryStatus}`}
                    role="status"
                    aria-label={`Auto-Router learning evidence status: ${routerLearningEvidenceStatusLabel}`}
                  >
                    {routerLearningEvidenceStatusLabel}
                  </p>
                )}
              ...
              {arCandidates.map((c, i) => {
                ...
                const routerCandidateEvidence = routerCandidateEvidenceBuilder.forModel(c.modelId);
                ...
                <div
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div role="list" aria-label={`${c.modelId} Auto-Router evidence badges`} style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                      ...
                      {routerCandidateEvidence && (
                        <span
                          role="listitem"
                          className={`pm-evidence-cue settings-router-evidence-cue pm-evidence-cue-${routerCandidateEvidence.tone}${routerCandidateEvidence.stale ? ' pm-evidence-cue-stale' : ''}`}
