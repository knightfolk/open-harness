...
- Tool-error recovery memory writes an append-only run-level ledger under `~/.openharness/router-learning/tool-error-ledger.jsonl` and can synthesize log-derived rows from reconstructed run traces, so routing can compare first-call failures against later recovery paths (`model/tool/provider`, retry distance, final answer outcome, evidence source, and session/run breadcrumbs).
- Use `GET /api/router/learning/tool-errors` to inspect this evidence in JSON.
- Prompt assembly now suppresses explicit "think step by step" triggers for prompt strategies with native or brief-private-plan reasoning contracts (for example OpenAI reasoning/Claude/Qwen/XML strategies), while keeping explicit step-by-step prompts available for strategies that still benefit from them.
...
npm run test:premier-stop-condition-audit
```
