...
- Tool-error recovery memory also writes an append-only run-level ledger under `~/.openharness/router-learning/tool-error-ledger.jsonl` so routing can compare first-call failures against later recovery paths (`model/tool/provider`, retry distance, final answer outcome, and session/run breadcrumbs).
